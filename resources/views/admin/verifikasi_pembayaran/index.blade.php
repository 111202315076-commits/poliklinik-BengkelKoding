            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        <th class="px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                            PASIEN
                        </th>
                        ...
                        <th class="px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-slate-400 text-center">
                            STATUS
                        </th>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    @forelse($tagihans as $tagihan)
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-[10px] text-slate-400 uppercase font-bold tracking-widest">
                                            RM : {{ $tagihan->daftarPoli->pasien->no_rm }}
                                        </span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <span class="font-bold text-slate-700 block text-sm">
                                    {{ $tagihan->daftarPoli->jadwalPeriksa->dokter->poli->nama_poli }}
                                </span>
                            ...
                            <td class="px-6 py-4">
                                <span class="text-xs font-bold text-slate-500 block uppercase">
                                    {{ \Carbon\Carbon::parse($tagihan->tgl_periksa)->translatedFormat('d F Y') }}
                                </span>
                            ...
                            <td class="px-6 py-4">
                                @if($tagihan->bukti_bayar)
                                    ...
                                    <img
                                        src="{{ asset('storage/'.$tagihan->bukti_bayar) }}"
                                        alt="Bukti Bayar"
                                        class="w-16 h-16 object-cover rounded-xl border border-slate-200 shadow-sm cursor-pointer hover:scale-110 transition duration-300"
                                        onclick="previewImage('{{ asset('storage/'.$tagihan->bukti_bayar) }}')">
                                ...
                                @if($tagihan->status_bayar=='menunggu_verifikasi')
                                    ...
                                    <form action="{{ route('admin.verifikasi_pembayaran.verifikasi',$tagihan->id) }}" method="POST">
                                        ...
                                        <button
                                            ...
                                            class="inline-flex items-center gap-2 px-6 py-3 bg-indigo-900 text-white 
                                            rounded-2xl text-[10px] font-black hover:bg-indigo-800 shadow-lg shadow-indigo-100 transition-all uppercase tracking-widest">
                                            KONFIRMASI LUNAS
                                        </button>
                                    </form>
    ...
    <div id="imageModal"
        ...
        <div class="relative">
            ...
            <button
                ...
                class="absolute -top-12 right-0 text-white text-5xl font-bold hover:text-red-400">
                &times;
            </button>
